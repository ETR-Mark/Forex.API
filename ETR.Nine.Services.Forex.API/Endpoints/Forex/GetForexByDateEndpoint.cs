...
public class GetForexByDateEndpoint : IEndpoint
{
    public void Map(IEndpointRouteBuilder app)
    {
        app.MapGet("/internal/forex/{date}", async (string date, string? c, IForexService forexService) =>
        {
            ...
            if(!isValidDate) return Results.BadRequest(new { Code = "INTERNAL_ERROR", Description = "Invalid date format. Use DDMMYYYY."});

            var result = await forexService.GetForexByDate(parsedDate, c ?? "USD"); 

            if (!result.Successful) return Results.BadRequest(new { Code = "FOREX-455", Description = result.Error?.Message ?? "Unknown error"});
